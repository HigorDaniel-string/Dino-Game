...
class DinoVoador(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.rect = self.image.get_rect()
        posicao = choice([1, 3])
        self.rect.center = (largura, altura - 94)
        self.rect.x = largura
...
todas_as_srites = pygame.sprite.Group()
dino = Dino()
todas_as_srites.add(dino)

for i in range(4):
    nuvem = Nuvens()
    todas_as_srites.add(nuvem)

for i in range(largura*2//64):
    chao = Chao(i)
    todas_as_srites.add(chao)

cacto = Cacto()
todas_as_srites.add(cacto)

dino_voador= DinoVoador()
todas_as_srites.add(dino_voador)
...
while True:
    ...
    if colidiu == True:
        ...
        pontos += 1
        todas_as_srites.update()
        texto_pontos = exibe_mensagem(pontos, 40, (0,0,0))

    if pontos % 100 == 0:
        som_pontuacao.play()
        if velocidade_jogo >= 29:
            velocidade_jogo += 0
        else:
            velocidade_jogo += 1
